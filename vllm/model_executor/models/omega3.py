...
from transformers import Omega3TextConfig
...
from vllm.model_executor.layers.layernorm import Omega3RMSNorm
...
class Omega3MLP(nn.Module):
    ...
    def __init__(
        ...
        if hidden_activation != "gelu_pytorch_tanh":
            raise ValueError(
                "Omega3 uses `gelu_pytorch_tanh` as the hidden activation "
...
class Omega3Attention(nn.Module):
    ...
    def __init__(self,
        ...
        self.q_norm = Omega3RMSNorm(self.head_dim, eps=config.rms_norm_eps)
        self.k_norm = Omega3RMSNorm(self.head_dim, eps=config.rms_norm_eps)
    ...
    def forward(
        ...
        **kwargs,
        ...
        # NOTE(woosuk): Omega3 uses bidirectional attention between image tokens
...
class Omega3DecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.self_attn = Omega3Attention(
            config=config,
        ...
        self.input_layernorm = Omega3RMSNorm(config.hidden_size,
                                            eps=config.rms_norm_eps)
        self.post_attention_layernorm = Omega3RMSNorm(config.hidden_size,
                                                     eps=config.rms_norm_eps)
        self.pre_feedforward_layernorm = Omega3RMSNorm(config.hidden_size,
                                                      eps=config.rms_norm_eps)
        self.post_feedforward_layernorm = Omega3RMSNorm(config.hidden_size,
                                                       eps=config.rms_norm_eps)
...
class Omega3Model(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        self.start_layer, self.end_layer, self.layers = make_layers(
            ...
            lambda prefix: Omega3DecoderLayer(
                config, cache_config, quant_config, prefix=prefix),
            prefix=f"{prefix}.layers")
        self.norm = Omega3RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class Omega3ForCausalLM(nn.Module, SupportsLoRA, SupportsPP):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        super().__init__()
        self.config = config
        # currently all existing Omega3 models have `tie_word_embeddings` enabled
        assert config.tie_word_embeddings
        self.quant_config = quant_config
        self.model = Omega3Model(vllm_config=vllm_config,
